...
machine) without editing the file, via environment variables of the form::
    ...
    CVF_<SECTION>__<KEY>=value        e.g.  CVF_PATHS__DATA_ROOT=/content/data
    CVF_<SECTION>__<SUB>__<KEY>=value e.g.  CVF_SEARCH__WEIGHTS__OCR=0.5
...
def find_settings_file(explicit: str | os.PathLike | None = None) -> Path | None:
    """Locate settings.yaml: explicit arg > $CVF_SETTINGS > repo configs/ > cwd."""
    candidates: list[Path] = []
    if explicit:
        candidates.append(Path(explicit))
    if os.environ.get("CVF_SETTINGS"):
        candidates.append(Path(os.environ["CVF_SETTINGS"]))
...
def load_settings(path: str | os.PathLike | None = None) -> Settings:
    """Load YAML settings, apply CVF_* env overrides, return typed Settings."""
