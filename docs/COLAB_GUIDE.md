...
|---|---|---|
| `GEMINI_API_KEY` | Nên có | dịch + mô tả lại truy vấn, VQA, VLM rerank, trợ lý KIS-C. Model theo `configs/settings.yaml`: `gemini-3.5-flash`, tự fallback `gemini-3-flash-preview` → `gemini-2.5-flash`; hết cả chuỗi → Google Translate → raw query (không bao giờ chặn thi đấu) |
