...
10. **Ops:** `scripts/26_run_ablations.py` (trận A1–A10 một lệnh, chấm bằng scorer
    ...
    `scripts/51_warm_cache.py` (làm ấm cache đúng chuỗi round-time) · `scripts/41_diff_submissions.py` (diff 2 lần chạy) · CLI `cvp eval` ·
...
13. **Test tầng tích hợp**: SearchEngine end-to-end với fake model (6 tests), official
    scorer 60+ tests, packager 34 tests... tổng cộng **568 tests** (Perfect V1: 316 kế thừa + 252 mới cho cross-rerank/objects-parquet/keyframe-align/service/VQA-đa-khung/temporal-boost/confidence-retry) + GitHub Actions CI.
...
```
python scripts/23_dump_signals.py --query-dir queries/dev-2025-finals          # engine chạy 1 lần/câu
python scripts/21_tune_weights.py --signals-dir artifacts/signal_dumps/dev-2025-finals \
       --gt queries/dev-2025-finals/gt.json --trials 60                        # tối ưu offline, in env CVP_...
```
...
├── tests/   568 tests CPU thuần (không cần GPU/mạng/data thật)
└── .github/workflows/ci.yml
```

## 6. Quy trình A→Z

1. **Local**: `pip install -e ".[search,dev]"` → `pytest` (bộ đầy đủ **568 pass** khi có torch-cpu như CI; không torch thì các test training/model bị skip — đo lại con số torch-less sau khi cài).
...
- ✅ Source + 568 tests + CI + 3 notebooks + docs (11 file) + app + service + report/ LaTeX kit.
