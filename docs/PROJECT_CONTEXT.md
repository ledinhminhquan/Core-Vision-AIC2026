...
1. **Cross-encoder rerank theo cặp** (`search/cross_rerank.py`) — công thức vô địch
   ...
   `qwen_reranker`** = Qwen3-VL-Reranker-8B (01/2026 — cross-encoder đa phương thức
...
9. **Chuỗi fallback Gemini** `gemini-3.7-flash → gemini-3.5-flash → flash-latest`
...
| VQA | Gemini 3.7 Flash (fallback 3-flash-preview → flash-latest) → Vintern-1B-v3.5 (offline) | trả lời theo NHÓM ứng viên, mỗi nhóm MỘT dải nhiều frame (`vqa.frames_per_answer`) |
