...
dưới 1 giây/truy vấn; 970 test CPU + CI đảm bảo tính đúng của công thức chấm
...
sơ tuyển diễn ra tháng 8/2026 trên hệ thống nộp bài của BTC (kết quả 30/8); chung kết on-site 12--26/9/2026;
...
\texttt{qwen\_reranker} (\texttt{Qwen/Qwen3-VL-Reranker-8B}, model 01/2026, chấm cặp
...
$\rightarrow$ \texttt{gemini-flash-latest}, offline thì rơi về Vintern-1B; answer luôn được
...
\texttt{CVP\_SEARCH\_\_WEIGHTS\_\_*} để dán sang máy thi --- không tốn lượt nộp nào của hệ thống nộp bài của BTC.
...
Chất lượng kỹ thuật của mã được găm bằng 970 test CPU thuần (không cần GPU/mạng/data thật)
...
# zip đầu ra CHỨA folder 'submission' đúng thể lệ hệ thống nộp bài của BTC, kèm MANIFEST sha256;
